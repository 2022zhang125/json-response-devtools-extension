...
function isFetchRequest(request) {
  // Chrome DevTools HAR 里通常会带 _resourceType
  // fetch 请求一般是 "fetch"，XHR 一般是 "xhr"
  if (request._resourceType) {
    return request._resourceType === "fetch";
  }

  // 兜底：部分版本可能没有 _resourceType，可以先不展示
  return false;
}
...
function createPrimitiveValue(value) {
  ...
  if (typeof value === "number") {
    ...
    numberElement.addEventListener("dblclick", () =>
  ...
  if (typeof value === "boolean") {
    ...
    booleanElement.addEventListener("dblclick", () =>
      copyText(String(value), "Value copied"),
    );
  ...
  if (value === null) {
    ...
    nullElement.addEventListener("dblclick", () =>
