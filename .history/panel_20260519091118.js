...
chrome.devtools.network.onRequestFinished.addListener((request) => {
  ...
  request.__createdAt = Date.now();
  request.__relativeTimeText = getRelativeTimeText(request.__createdAt);
...
function renderRequestList() {
  ...
  requests.forEach((request) => {
    ...
    name.title = getRequestName(request.request.url);

    const time = document.createElement("span");
    time.className = "request-time";
    time.textContent = getRelativeTimeText(request.__createdAt);
    time.title = request.__createdAt
      ? new Date(request.__createdAt).toLocaleString()
      : "";

    const status = document.createElement("span");
    status.className = `request-status ${getStatusClass(
      request.response.status,
    )}`;
    status.textContent = request.response.status || "-";
    status.title = `${request.response.status || "-"} ${
...
function getRequestName(url) {
  try {
    ...
    return lastPath || parsedUrl.hostname;
  } catch {
    return url;
  }
}
