...
> 面向后端接口调试的 Chrome / Edge DevTools 扩展。以树形结构展示 JSON 响应，支持多项目 Swagger 跳转与响应解密。
...
### JSON 树形查看器
- 自动拦截页面所有 XHR / Fetch 请求，过滤并展示 JSON 响应
- 可折叠树形结构，层次清晰，支持大数据量浏览
- 双击任意 **Key**、**Value** 或 **对象/数组摘要** 直接复制内容
- URL 类型字符串渲染为可点击链接
- `Ctrl/Cmd + A` 一键复制当前完整响应（格式化 JSON）
...
### 多项目 Swagger 联动
- 点击接口名称自动匹配对应项目的 Swagger 地址并跳转
- 根据 API 路径前缀智能识别归属项目，无需手动切换
- 跳转同时将接口路径复制到剪贴板，方便 Swagger 内二次搜索

### 响应解密
- 支持 SM4（国密）和 AES-128 ECB 两种算法
- 自动解密指定字段（默认 `data`）并以树形展示解密内容
...
| API 路径前缀 | 用于匹配请求归属项目（每行一个） | `/loancrm-admin` |

点击接口名跳转时，自动按**最长前缀匹配**规则选择对应 Swagger 配置。
...
2. 刷新页面或正常操作，左侧列表自动收集 JSON 请求
3. 点击任意请求，右侧展示树形 JSON 内容
4. 在顶部搜索框输入关键词，实时高亮定位
