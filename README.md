...
- 拖拽调整面板宽度，可折叠/展开以获得更大浏览区域；向左拖到 120px 以内（或直接拖出面板左边缘）松手即动画收起，其余位置松手就停在原地
- 一键清空全部请求记录
- 顶部搜索框的关键词会实时过滤列表（按接口路径匹配），标题旁显示命中数 / 总数
- 配置 API 路径前缀后，自动过滤无关第三方请求，只捕获匹配的接口

### 双 Tab 搜索 + 请求过滤
- 顶部搜索框同时做两件事：**高亮当前面板内容** + **按接口路径过滤左侧请求列表**
  - 输入 `/xxx/aaa/ccc` 即可从上百个请求中筛出目标接口，标题旁显示「命中数 / 总数」
  - 过滤同时匹配「剥掉前缀后的接口路径」与「原始请求路径」，所以关键词落在被剥掉的前缀里也能搜到
  - 同样遵循 `Aa` / 全词 / `.*` 三个开关；`.*` 模式下正则敲到一半无法编译时，沿用上一个有效模式，不会把整个列表放回来
  - **当前选中的请求始终保留**，所以搜索响应体里的值时不会把正在查看的请求筛掉；不匹配时它会置灰并标注「当前查看 · 不匹配筛选」，且不计入命中数
  - 无命中时列表给出提示，`Esc` 或清空关键词即可恢复完整列表
...
### 配置一键导入 / 导出（导出即绑定，更新后自动恢复）
扩展的配置存在扩展 ID 名下，**从新目录加载或先移除再加载，Chrome 会视为另一个扩展，配置随之清空**。所以导出的这份 JSON 同时就是「设置文件」：

- 设置页顶部按钮将全部配置（Swagger、点击跳转开关、解密开关、目标字段、密钥列表）导出为 JSON 文件
- 导出或导入之后会**记住这个文件**，此后 Swagger 配置、解密密钥、各开关的每次变更都自动写回它
- 打开设置页时：本地已有配置 → 刷新文件；本地为空（刚更新 / 新装）→ 自动从文件恢复，不用再手动导入
- 导入为**合并**模式：相同配置按业务身份去重更新，不重复添加；非本扩展导出的文件会被拒绝
- 文件句柄存在 IndexedDB 中，浏览器重启后仍有效；权限过期时设置页会出现「重新授权并同步」按钮
- 不支持 File System Access 的浏览器自动退回一次性的下载 / 选择文件，功能不受影响
- 更新最省事的方式仍是**覆盖原来的扩展目录后点「重新加载」**，此时扩展 ID 不变，配置原样保留
- ⚠️ 该文件包含**解密密钥明文**，请通过可信渠道传输，勿公开分享
...
| 快捷键 | 功能 |
|--------|------|
| `Ctrl/Cmd + F` | 聚焦搜索框（高亮面板内容 + 按路径过滤请求列表） |
| `Enter` | 跳转到下一个匹配项 |
...
├── settings-file.js       # 设置文件绑定（File System Access + IndexedDB 句柄持久化）
