...
### 设置文件（更新后保留个性化设置）
扩展的配置存在扩展 ID 名下，**从新目录加载或先移除再加载，Chrome 会视为另一个扩展，配置随之清空**。设置页提供「设置文件」绑定来兜底：

- 绑定磁盘上的一个 JSON 文件后，Swagger 配置、解密密钥、跳转开关等**每次变更自动写入该文件**
- 打开设置页时：本地已有配置 → 刷新文件；本地为空（刚更新 / 新装）→ 自动从文件恢复，无需再手动导入
- 更新后如果配置被清空，只需重新指向同一个文件，一键找回全部设置
- 绑定已有文件时采用**合并**策略，文件与本地的配置都不会被覆盖丢失
- 文件句柄存在 IndexedDB 中，浏览器重启后仍然有效；出于安全限制可能需要点一次按钮重新授权
